...
| Token | 值 | 用途 |
|---|---|---|
| `--bg-base` | `#08080A` | 窗口底色（标题栏、侧栏最底层） |
| `--bg-raised` | `#0D0D10` | 主内容区（聊天区、面板页） |
| `--bg-overlay` | `#131316` | 卡片、用户气泡、悬浮块 |
| `--bg-float` | `#18181C` | Tasks 面板、输入框、popover |
| `--bg-hover` | `rgba(255,255,255,.03)` | 行 hover |
| `--border-subtle` | `rgba(255,255,255,.055)` | 仅用于必要的分隔（导航下缘、卡片描边） |

> 明度阶梯：08 → 0D → 13 → 18，每级 ~5% 明度差，整体压深后层级仍靠"面"可辨。分隔线大幅减少，让层级靠"面"表达。

### 3.2 品牌绿（收口后）

| Token | 值 | 用途 |
|---|---|---|
| `--accent` | `#3FD07A` | 状态点、主按钮、激活指示条 |
| `--accent-dim` | `rgba(63,208,122,.12)` | 激活态底色（nav pill、会话行） |
| `--accent-text` | `#6FD89B` | 需要绿色的文字（如 connected 状态） |

> 从 `#5DC63B` 微调至 `#3FD07A`：压深背景后降低绿的刺激感，保持同色族统一选中态/徽章/状态点。

### 3.3 文字色阶（6 级）

| Token | 值 | 用途 |
|---|---|---|
| `--text-1` | `#E2E2E6` | 页面标题、会话标题 |
| `--text-2` | `#C4C4CA` | 正文、消息内容 |
| `--text-3` | `#8C8C94` | 次要说明、参数文本 |
| `--text-4` | `#606068` | 时间戳、辅助信息、placeholder |
| `--text-5` | `#44444B` | 禁用态 |
...
| Token（暗 → 亮） | 亮色值 | 说明 |
|---|---|---|
| `--bg-base / raised / overlay / float` | `#EFEFF2 / #FAFAFB / #F1F1F4 / #FFFFFF` | 亮色下层级反转：内容区最亮，侧栏略灰，浮层纯白靠阴影托起 |
| `--text-1~5` | `#17171B / #34343B / #5E5E66 / #8E8E96 / #B9B9BF` | 六级灰阶 |
| `--accent / dim / text` | `#1F9D5A / rgba(31,157,90,.10) / #177A48` | 白底上绿色必须加深降纯，否则发飘 |
| `--on-accent` | `#FFFFFF`（暗色为 `#08080A`） | Send 按钮上的箭头色 |
...
| `--tasks-glow` | `radial-gradient(ellipse at right, rgba(31,157,90,.07), transparent 60%)` | Tasks 面板右侧辉光，亮色改为极浅绿晕 |
...
阴影:
  ...
  --glow-accent:  0 0 10px rgba(63,208,122,.28)   （仅状态点、运行指示）
```
...
| 底部状态栏 | 高 32px 上缘 1px `--border-subtle`：6px `--accent` 呼吸点 + `connected :3000` 12px tabular `--text-4`；断线时点变 `#E5484D` + 文案 `--text-3` |
...
| 元素 | 规格 |
|---|---|
| 容器 | 底 `--bg-raised`；消息列 `max-width: 760px` 居中，水平 padding 32px |
| Agent 头（Manager） | sticky 顶部位：20px 圆形 agent 头像位（`--accent-dim` 底 + glyph）+ 名称 15px/600 `--text-1` + 右侧状态（运行中=绿点+"Running" 12px `--accent-text`）。**sticky 必须实底**：`--bg-raised` 背景 + 下缘 12px 渐变遮罩（或 backdrop-blur 8px），防止消息滚动穿透 |
...
| 项 | 规格 |
|---|---|
| 定位 | 不再是内容流的一部分：**悬浮于输入框上方 12px**，与输入框同宽（`max-width 760px` 对齐） |
| 容器 | `--bg-float` + `--shadow-float` + 圆角 16px + 右侧绿色辉光（沿用任务面板文档 §3.1 `--panel-glow`） |
| 出现/消失 | 有任务时自输入框上缘滑入（translateY 8px→0 + fade 200ms）；全部完成 5s 后自动折叠为一行摘要（"✓ 3 tasks · 42s"），点击重新展开 |
| 与输入框关系 | 二者共享 `--bg-float` 色族与 16px 圆角体系，视觉上成"输入区组合件"而非游离卡片 |

### 4.7 输入框（Composer / InputBar）

结构：**外层容器 = textarea + 底部工具条**，与 Tasks 面板组成"输入区组合件"（§4.6，同宽 760px、间距 12px）。

| 元素 | 规格 |
|---|---|
| 容器 | `--bg-float` + 圆角 16px + 1px `--border-subtle`；无独立阴影（Tasks 面板悬浮时才用 `--shadow-float`） |
| 容器：聚焦 | 描边 `var(--composer-focus)`（暗 `rgba(63,208,122,.35)` / 亮 `rgba(31,157,90,.45)`）+ 外圈 3px 光晕 `var(--composer-halo)`；背景不变，过渡 150ms |
...
| **Send 主按钮** | 32×32，圆角 8px，`--accent` 底 + `var(--on-accent)` 箭头（↑ 14px，stroke 1.6）——**不要写死 `#0C0C0E`**：暗色 `--on-accent = #08080A`、亮色 `--on-accent = #FFFFFF`；hover 仅 105% 亮度（filter brightness(1.05)），无位移 |
...
选中文本: background rgba(63,208,122,.2)
焦点环: 所有可聚焦元素 outline: 2px rgba(63,208,122,.4)，仅 :focus-visible
```
...
- Tasks 面板浮到输入框上方后，注意消息列表底部要留 `padding-bottom = 面板高 + 12px`，并在面板出现/消失时平滑过渡，避免遮挡最后一条消息。

---

## 7. 验收清单

- [ ] 四级背景（08/0D/13/18）全量替换，页面无 `#101010` 平铺感
