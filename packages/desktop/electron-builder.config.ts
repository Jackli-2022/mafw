...
const extraResources: Array<{ from: string; to: string; filter?: string[] }> = [
  ...
  // Runtime icons live at process.resourcesPath/icons (windows.ts iconsDir(),
  // tray + BrowserWindow icon paths) — asar-only packing via `files` is not
  // reachable from there. Generated by predev/prebuild; win.icon already
  // requires this directory at package time, so no existsSync guard.
  {
