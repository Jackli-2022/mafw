...
test('current command surface is fully covered', () => {
  ...
  assert.deepEqual(names, ['btw', 'compact', 'diff', 'editor', 'export', 'focus', 'fork', 'help', 'model', 'new', 'older', 'queue', 'redo', 'rename', 'sessions', 'status', 'undo', 'verbose', 'waitwhat'])
})
...
test('immediate commands (busy 时不排队立即执行) are flagged', () => {
  ...
  assert.deepEqual(immediate, ['compact', 'diff', 'editor', 'export', 'focus', 'fork', 'help', 'model', 'older', 'queue', 'rename', 'sessions', 'status', 'verbose', 'waitwhat'])
})
