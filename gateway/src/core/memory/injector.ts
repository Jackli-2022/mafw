import { Delta, ConstraintDelta, PromptDelta, PatternDelta, InjectionResult, TriggerCondition } from '../types/parametric';
 ...
 */
export class DeltaInjector {
  private readonly MAX_DELTA_COUNT = 5;
  ...
  render(injection: InjectionResult): string {
    if (injection.injected.length === 0) return '';
    ...
    for (const d of injection.injected) {
      ...
      if (d.type === 'constraint') {
        body = (d as ConstraintDelta).rule;
      } else if (d.type === 'prompt') {
        body = (d as PromptDelta).prompt_delta;
      } else if (d.type === 'pattern') {
        body = (d as PatternDelta).pattern_template;
      }
      // 截断单行显示
   ...
   */
  checkPollution(agentOutput: string): { clean: boolean; violations: string[] } {
   ...
   */
  private estimateTokens(d: Delta): number {
    let text = '';
    if (d.type === 'constraint') text = (d as ConstraintDelta).rule;
    else if (d.type === 'prompt') text = (d as PromptDelta).prompt_delta;
    else if (d.type === 'pattern') text = (d as PatternDelta).pattern_template;
