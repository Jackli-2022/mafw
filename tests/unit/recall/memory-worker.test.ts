...
describe('MemoryWorker (persistent LLM channel)', () => {
  ...
  beforeEach(() => {
    ...
    fakeClient = {
      session: {
        create: jest.fn().mockImplementation(async () => ({ id: `w-${++counter}` })),
        prompt: jest.fn(),
  ...
  test('reuses the created session across prompts', async () => {
    fakeClient.session.prompt.mockResolvedValue({ parts: [{ type: 'text', text: 'ok' }] });
  ...
  test('invalidates the session on failure and re-creates on next call', async () => {
    fakeClient.session.prompt
      ...
      .mockResolvedValueOnce({ parts: [{ type: 'text', text: 'recovered' }] });
  ...
  test('session is persistent (no rotation across prompts)', async () => {
    ...
    fakeClient.session.prompt.mockResolvedValue({ parts: [{ type: 'text', text: 'x' }] });
    await w.prompt('1');
    await w.prompt('2');
    await w.prompt('3');
  ...
  test('prompt includes pinned model when passed as argument', async () => {
    ...
    expect(fakeClient.session.prompt).toHaveBeenCalledWith(
      expect.objectContaining({ model: pinned }),
    );
  });

  test('prompt omits model field when not passed', async () => {
    fakeClient.session.prompt.mockResolvedValue({ parts: [{ type: 'text', text: 'ok' }] });
  ...
  test('returns joined text parts only', async () => {
    fakeClient.session.prompt.mockResolvedValue({
      parts: [{ type: 'text', text: 'a' }, { type: 'tool', text: 'b' }, { type: 'text', text: 'c' }],
    });
  ...
  test('summarizes an idle session before the next prompt and continues', async () => {
    ...
    expect(fakeClient.session.summarize).toHaveBeenCalledWith(
      expect.objectContaining({
        sessionID: expect.stringMatching(/^w-/),
        providerID: 'xiaomi',
        modelID: 'mimo-v2.5',
      }),
    );
  ...
  test('rotates session when summarize fails', async () => {
    ...
    fakeClient.session.summarize.mockResolvedValue(true);
