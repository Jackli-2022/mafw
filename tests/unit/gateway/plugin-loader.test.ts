...
describe('PluginLoader', () => {
  ...
  test('getAdapters returns only ok adapters', async () => {
    fs.writeFileSync(path.join(tmpDir, 'good.js'), `
      module.exports = { name: 'good', plan: 'Good', async fetch() { return null; } };
    `);
    fs.writeFileSync(path.join(tmpDir, 'bad.js'), 'syntax error');
    await loader.init();
    const adapters = loader.getAdapters();
    expect(adapters).toHaveLength(1);
    expect(adapters[0].name).toBe('good');
  });

  test('reload rescans directory', async () => {
    await loader.init();
    expect(loader.getState()).toHaveLength(0);
    fs.writeFileSync(path.join(tmpDir, 'late.js'), `
      module.exports = { name: 'late', plan: 'Late', async fetch() { return null; } };
    `);
    await loader.reload();
    expect(loader.getState()).toHaveLength(1);
    expect(loader.getState()[0].name).toBe('late');
  });
});
