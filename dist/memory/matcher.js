...
class TriggerMatcher {
    ...
    match(condition, context) {
        ...
        if (condition.keywords && context.goalKeywords) {
            const hasMatch = condition.keywords.some(kw => context.goalKeywords.some(gk => gk.toLowerCase().includes(kw.toLowerCase())));
            if (!hasMatch)
                return false;
        }
        // 6. affected_file_pattern（glob 匹配）
        if (condition.affected_file_pattern && context.affectedFiles) {
            const hasMatch = condition.affected_file_pattern.some(pat => context.affectedFiles.some(f => this.globMatch(f, pat)));
            if (!hasMatch)
                return false;
        }
        return true;
...
exports.TriggerMatcher = TriggerMatcher;
