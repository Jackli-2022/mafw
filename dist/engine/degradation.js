...
exports.DegradationStrategy = void 0;
...
class DegradationStrategy {
    ...
    constructor(projectDir = '.') {
        ...
        this.reportGenerator = new report_generator_1.ReportGenerator(projectDir);
    }
    /**
     * 判断是否需要 L1 警告
     */
    shouldWarn(loopCount, maxLoops) {
        return loopCount >= Math.ceil(maxLoops * 0.6); // 3/5
    }
    /**
     * L1 警告处理：提示用户选择
     */
    async handleL1(goalId, loopCount, reviewResult) {
        const warning = [
     ...
     * L2 降级：自动执行
     */
    async autoDegrade(goalId, executeResult) {
     ...
     * L2 降级：接受部分完成
     */
    async degradeToPartial(goalId, executeResult) {
     ...
     * L3 熔断检测：连续 3 轮相同原因
     */
    async checkL3Oscillation(goalId, lessons) {
     ...
     */
    async recoverFromCheckpoint(goalId, checkpointPath) {
