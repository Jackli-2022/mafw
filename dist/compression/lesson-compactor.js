...
exports.LessonCompactor = void 0;
/**
...
class LessonCompactor {
    ...
    summarizeLesson(lesson) {
        ...
        const core = sentences.filter(s => /(必须|禁止|应该|不得|需要|需|务必|严禁|确保|检查|验证|引用|查阅|对比|禁止|不能|不准|不允许|must|must not|should|need to|ensure|check|verify|reference|compare)/i.test(s));
        if (core.length > 0) {
     ...
     * 压缩完整性验证（5 项检查清单）
     */
    verifyCompaction(original, compacted) {
        const errors = [];
        // 1. loop 不能丢失
        if (compacted.loop !== original.loop)
            errors.push('loop 字段丢失');
        // 2. result 不能丢失
        if (compacted.result !== original.result)
            errors.push('result 字段丢失');
        // 3. domain 不能丢失
        if (compacted.domain !== original.domain)
            errors.push('domain 字段丢失');
...
exports.LessonCompactor = LessonCompactor;
//# sourceMappingURL=lesson-compactor.js.map