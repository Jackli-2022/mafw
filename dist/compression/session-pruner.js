...
class SessionPruner {
    ...
    truncateToolOutput(output) {
        ...
        if (lines.length <= this.config.toolOutputReserve)
            return output;
    ...
    renderDigests(digests) {
        ...
        for (const d of digests) {
            lines.push(`[WAVE_DIGEST id=${d.id} status=${d.status} tokens_saved=${d.tokens_saved}]`, `  domain: ${d.domain}`, `  completed_tasks: [${d.completed_tasks.join(', ')}]`, `  key_decisions: [${d.key_decisions.join(', ')}]`, `  blockers: [${d.blockers.join(', ')}]`, `  files_changed: [${d.files_changed.join(', ')}]`);
        }
        return lines.join('\n');
    }
    /**
...
//# sourceMappingURL=session-pruner.js.map