...
| Agent 头（Manager） | sticky 顶部位：20px 圆形 agent 头像位（`--accent-dim` 底 + glyph）+ 名称 15px/600 `--text-1` + 右侧状态（运行中=绿点+"Running" 12px `--accent-text`） |
...
| 现状 | 改造 |
|---|---|
| 灰底薄边框 + Send 像禁用 | 容器：`--bg-float` + 圆角 16px + 1px `--border-subtle`；聚焦时描边变 `rgba(63,208,122,.35)` + 外圈 3px `rgba(63,208,122,.09)` 光晕 |
| | 内高最小 52px、最大 200px 自动生长；placeholder 14px `--text-4` |
| | 左下：`+` 附件 / `@` 引用 agent 图标按钮（24px，`--text-4` hover `--text-2`）；右下：模型选择 pill（12px `--text-3` `--bg-overlay`）+ **Send 主按钮**：32px 圆角 8px，`--accent` 底 + `#0C0C0E` 文字/箭头；空输入时 40% 透明度禁用（底色仍在，明确它是主操作） |
| | 快捷键提示：聚焦时右下淡显 `⏎ Send · ⇧⏎ New line` 11px `--text-5` |
