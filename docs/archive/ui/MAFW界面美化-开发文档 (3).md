...
| `--bg-float` | `#1F1F26` | Tasks 面板、输入框、popover |
...
| 元素 | 规格 |
|---|---|
| 容器 | 底 `--bg-raised`；消息列居中、水平 padding 32px，**列宽随视口分档**（宽屏不再两侧大留白）：默认 `760px`，视口 ≥1500px → `880px`，≥1800px → `1000px`，≥2200px → `1100px`（上限保可读性，不再加宽）；Tasks 面板、Composer、AskCard/PermissionCard 全部跟随同一列宽 |
...
| 项 | 规格 |
|---|---|
| 定位 | 不再是内容流的一部分：**位于输入框上方 12px**，与输入框同宽（跟随消息列分档列宽，§4.5） |
...
结构：**外层容器 = textarea + 底部工具条**，与 Tasks 面板组成"输入区组合件"（§4.6，与消息列同宽、间距 12px）。
