...
## 6. 前端（Config Plugins 页）

### 6.1 行渲染分支（`e.builtin` 为 true 时）
...
| SDK | `plugins.list` 类型含新可选字段（types 测试断言函数存在即可，沿用现有 stub-fetch 模式） |
...
- gateway：`plugins/hub.ts`、`routes/plugins.ts`（仅 deps 透传）、`index.ts`（pluginHubDeps + cleanup 调用）、`runtime/loader.ts`、`media/media-plugin-loader.ts`、`usage/plugin-loader.ts`
